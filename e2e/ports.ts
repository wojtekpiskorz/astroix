...
// defaults stay off the canonical trio (main's manual `bun run dev` doubles
// as the dogfood server on the smoke port 4312, src-fixture's manual default
// is 4310) so a lane and a dogfood server never want the same port; this
// module owns the Playwright-driven defaults and the config always exports
// the var on that path.
