...
// (add and remove). The chrome-side half (the sidebar listing the new entry
// marker-free) is deliberately NOT asserted here: an external content add
// does not live-refresh the chrome's collections list today (astro's
// content event never reaches the client chrome; the builder's own writes
// refresh through the write loop) — a pre-existing gap filed separately
// from #119.
