...
METRICS {"programmaticBootMs":1831,"runnerListeners":{"listenerCountBefore":null,"listenerCountDuring":null,"listenerCountAfter":null},"managedConfiguredPort":0,"managedBoundPort":62458,"managedForegroundBootMs":1874,"managedForegroundSteadyRssKiB":274352,"managedForegroundRssSampling":{"intervalMs":300,"maxSamples":15,"windowSize":5,"tolerancePercent":2,"minimumToleranceKiB":4096,"samplesKiB":[354384,272432,274192,274352,274352,274352],"convergedWindowKiB":[272432,274192,274352,274352,274352],"toleranceKiB":5488,"steadyStateRssKiB":274352},"watchedDirectories":10,"watchedEntries":25,"watcherListeners":35,"managedForegroundShutdownMs":20,"managedForegroundShutdownEscalated":false,"astroVersion":"7.2.7","viteVersion":"8.2.2"}
```
...
- The proof directly spawned a foreground `astro dev --port 0 --host 127.0.0.1` child and owned its `ChildProcess`. Disposable observer evidence recorded `config.server.port` as `0`; the CLI URL and Vite `httpServer.address()` independently agreed on the actual bound port, 62458. The child served the plain project and exposed the watcher snapshot. RSS sampling ran every 300 ms for at most 15 samples and required a five-sample window whose span did not exceed the greater of 4096 KiB or 2% of its median. The run converged on `[272432, 274192, 274352, 274352, 274352]` KiB. Its 1920 KiB span was within the 5488 KiB tolerance, so the recorded steady-state value is the 274352 KiB window median. A bounded SIGTERM stopped the exact owned child in 20 ms without SIGKILL escalation.
