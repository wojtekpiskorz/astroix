...
METRICS {"programmaticBootMs":1278,"runnerListeners":{"listenerCountBefore":null,"listenerCountDuring":null,"listenerCountAfter":null},"managedConfiguredPort":0,"managedBoundPort":59757,"managedForegroundBootMs":1421,"managedForegroundSteadyRssKiB":255904,"managedForegroundRssSampling":{"intervalMs":300,"maxSamples":15,"windowSize":5,"tolerancePercent":2,"minimumToleranceKiB":4096,"samplesKiB":[347728,340192,339504,331056,331056,331056,331056,263008,263008,258384,255904,255904,255904,255904],"convergedWindowKiB":[258384,255904,255904,255904,255904],"toleranceKiB":5119,"steadyStateRssKiB":255904},"watchedDirectories":10,"watchedEntries":25,"watcherListeners":35,"managedForegroundShutdownMs":110,"managedForegroundShutdownEscalated":false,"astroVersion":"7.2.7","viteVersion":"8.2.2"}
```
...
- The proof directly spawned a foreground `astro dev --port 0 --host 127.0.0.1` child and owned its PID. Disposable observer evidence recorded `config.server.port` as `0`; the CLI URL and Vite `httpServer.address()` independently agreed on the actual bound port, 59757. The child served the plain project and exposed the watcher snapshot. RSS sampling ran every 300 ms for at most 15 samples and required a five-sample window whose span did not exceed the greater of 4096 KiB or 2% of its median. The run converged on `[258384, 255904, 255904, 255904, 255904]` KiB. Its 2480 KiB span was within the 5119 KiB tolerance, so the recorded steady-state value is the 255904 KiB window median. A bounded SIGTERM stopped the child in 110 ms without SIGKILL escalation.
- Installed versions were Astro 7.2.7 and Vite 8.2.2.
- Every acquired server, browser, proxy and managed foreground child becomes cleanup-owned before later assertions run. Commands and stop waits are bounded. Foreground-child shutdown sends SIGTERM, waits up to 5 s, then escalates to SIGKILL with a separate 3 s bound if needed. After resources stop, the runner requires five unchanged 100 ms temporary-tree intervals, within a 3 s bound, before deletion. The proof disables Astro's asynchronous update check so it cannot recreate `.astro/settings.json` after the runner has removed the temp root. A work failure plus a teardown failure prints every cause and retains the temporary project instead of deleting it under a possibly live process. Forced failures immediately after the plain server/proxy start and the managed foreground child start both exited `1`; their cleanup completed, removed both temporary roots and left no managed child PID.
...
10. Managed-process metrics parse the actual URL from foreground CLI output, compare its port with observer access to Vite's `httpServer.address()`, use macOS `ps` for the directly spawned child RSS, and manage that child with signals. The steady-state criterion is an operational proof threshold, not an Astro guarantee: five consecutive samples, 300 ms apart, must fit within the greater of 4096 KiB or 2% of their median, with a hard limit of 15 samples.
