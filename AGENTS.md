...
- `npm run test:e2e` — during the no-E2E interval (ADR-0010, amended 2026-09-01 by owner ruling on #197): the serverless plain-fixture build smoke only. The legacy-chrome suite is deleted; the B-lane capture suites and the web-host lanes (#240) rebuild product e2e. Port ownership rules stay in `e2e/ports.ts` for the capture suites (#120).
