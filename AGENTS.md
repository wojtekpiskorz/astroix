...
- `npm run test:e2e` — during the no-E2E interval (ADR-0010, amended 2026-09-01 by owner ruling on #197): the serverless plain-fixture build smoke, the frozen-contract suites (B1/B2, chromium oracle-boot freeze legs), and the retained-UI regression (C2, `e2e/retained-ui.spec.ts` — the real chrome over adapters, disposable oracle boot). None of these is product E2E; the web-host lanes (#240) rebuild that. Port ownership rules stay in `e2e/ports.ts` for the oracle-boot suites (#120).
