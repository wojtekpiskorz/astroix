...
- `npm test` — vitest + happy-dom, one root run: the retained unit lanes (`src/core` CRAP tooling, `packages/core`, `packages/protocol`, `packages/app-shell`), the frozen-contract schema validators (`e2e/behavior-contracts/schema/`), the presentation mounts (`e2e/retirement-readiness/presentation-mount.test.tsx`), and the five serverless retirement-readiness legs (`e2e/retirement-readiness/readiness.test.ts` — corpora validation, retained-UI coupling scan, fixture plainness + zero-byte build, counts, deletion inventory; the legs build the canonical fixture, so `e2e/fixture/` must be installed). `npm run test:watch` for watch mode.
