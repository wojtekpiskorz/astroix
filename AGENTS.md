...
- `npm test` — vitest + happy-dom: the retained unit lanes (`src/core` CRAP tooling, `packages/core`, `packages/app-shell`), the frozen-contract schema validators (`e2e/behavior-contracts/schema/`), and the five serverless retirement-readiness legs (`e2e/retirement-readiness/readiness.test.ts` — corpora validation, retained-UI coupling scan + mounts, fixture plainness + zero-byte build, counts, deletion inventory; the legs build the canonical fixture, so `e2e/fixture/` must be installed). `npm run test:watch` for watch mode.
...
- **Metric honesty**: full CRAP (CC² × (1−cov)³ + CC) only where per-function unit coverage is real (`src/core` tooling + `packages/core`); the shell tier (`packages/app-shell`) is a CC-only watchlist. The integration watchlist tiers (`src/node`, `src/client`) were deleted with their functions at the gate.
