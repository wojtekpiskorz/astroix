...
- `packages/runtime/` — born at #221 (D2): the registry persistence seam (canonical realpath identity, ProjectKey allocation, versioned-JSON store with quarantine; CRAP covered tier for `registry/`, later seams watchlist until their lanes rule). `ProjectRegistry`, `SessionSupervisor`, `ProjectRuntime`, `EditAuthority` are deep seams inside it per ADR-0006.
