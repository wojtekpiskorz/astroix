...
- `npm install` — install dependencies (`e2e/fixture/` is a standalone plain-Astro package with its own committed lockfile; the disposable oracle copies are generated by the staging scripts and gitignored).
...
- `npm run test:e2e` — during the no-E2E interval (ADR-0010, amended 2026-09-01 by owner ruling on #197): the serverless plain-fixture build smoke, the frozen-contract suites (B1/B2, chromium oracle-boot freeze legs), the retained-UI regression (C2, `e2e/retained-ui.spec.ts` — the real chrome over adapters, disposable oracle boot), and the retirement-readiness suite (A5, `e2e/retirement-readiness.spec.ts` — the aggregate deletion-eligibility proof over the fixture, contracts, retained UI, counts, and one oracle comparison; evidence report at `docs/retirement-readiness.md`). None of these is product E2E; the web-host lanes (#240) rebuild that. Port ownership rules stay in `e2e/ports.ts` for the oracle-boot suites (#120).
...
- Imports flow strictly downward: app shell (`app.tsx`, `sidebar.tsx`, `chrome.tsx`, `entry.tsx` + bootstrap helpers like `react-guard.ts`, `styles.ts`) → `features/<vertical>/` → shared modules (`canvas/`, `editor/`) → app-shell presentation (`packages/app-shell/src/presentation/`, props from frozen contracts) → `components/ui/` → `lib/`; `src/core`/`packages/core` is importable from anywhere, and the app store (`src/client/store.ts` → `packages/app-shell`) serves every layer except `components/ui/` and `lib/`. No sideways (feature ↔ feature, shared ↔ shared), no upward, no cycles.
