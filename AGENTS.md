...
- `packages/runtime/` — born at #221 (D2): the persistence + authority seams: `registry/` (D2: canonical identity, ProjectKey, versioned-JSON store with quarantine), `kernel-lease/` + `private-boot/` (D3: lifetime-held sqlite kernel leases, one-use boot capability over private IPC) — all CRAP covered tier; later seams watchlist until their lanes rule. `ProjectRegistry`, `SessionSupervisor`, `ProjectRuntime`, `EditAuthority` are deep seams inside it per ADR-0006.
