...
| **preflight** | The local CRAP hard stop (`npm run preflight`): a baseline ratchet over `src/` (tooling) + `packages/core` + `packages/protocol` + `packages/app-shell` + `packages/runtime/registry/` (future packages join in their landing PR) — every run fails any new stop breach, complexity or coverage regression alike; the agent runs it before `gh pr create`. |
