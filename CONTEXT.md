...
| **preflight** | The local CRAP hard stop (`npm run preflight`): a baseline ratchet over `src/` + `packages/core` (future workspace packages join in their landing PR) — every run fails any new stop breach, complexity or coverage regression alike; the agent runs it before `gh pr create`. |
