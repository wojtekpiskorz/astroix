...
| **content-sync leg** | Which watcher half a `astroix:content-synced` push carries: **srcdir** (pre-commit file event) or **loader** (post-commit data-store write). The chrome sequences on it — srcdir invalidates the content caches immediately, loader holds until the canvas's next load so its refetch never races the post-commit full-reload render (#155). |
