...
| **preflight** | The local CRAP hard stop (`bun run preflight` until the npm lane lands): a full-source baseline ratchet — every run evaluates the whole source tree against the baseline and fails any new stop breach, complexity or coverage regression alike; the agent runs it before `gh pr create`. |
