...
| **raw mode** | A pane's free-form editing mode — the value(s) edited as plain text instead of structured widget rows: the CSS rule editor's declarations as plain CSS text, the content entry form's values as the raw truth pane (#252). |
...
| **preflight** | The local CRAP hard stop (`npm run preflight`): a baseline ratchet over `src/` (tooling) + `packages/core` + `packages/protocol` + `packages/app-shell` + `packages/runtime/{registry,kernel-lease,private-boot}/` (future packages join in their landing PR) — every run fails any new stop breach, complexity or coverage regression alike; the agent runs it before `gh pr create`. |
