...
export async function runMatrix(input: MatrixInput): Promise<MatrixResult> {
  ...
  const manifest: CandidateManifest = {
    ...
    source: input.source,
    pins: input.pins,
    build: { ...input.build, builtOnce: true },
    transfer: {
      mode: input.mode,
  ...
  /** The received bytes as a tester would hold them: the dry run's staged copy, or the workflow's download. */
  let receivedZip = input.zipPath;

  try {
    // ——— the transfer law: one checksum across assembled/received ———
    ...
    (manifest.transfer as { checksumAfter: string }).checksumAfter = receivedFacts.sha256;
    (manifest.transfer as { match: boolean }).match = transfer.ok;
    if (!transfer.ok) {
      failures.push(
    ...
    // ——— leg: unsupported-node-sass (prelaunch, never installed) ———
    ...
    (manifest.fixtures.nodeSass as { rejected: boolean }).rejected = sass.ok;
    (
      manifest.fixtures.nodeSass as { diagnostic: Readonly<Record<string, unknown>> | null }
    ).diagnostic = sass.diagnostic;
    ...
    // ——— leg: L1 black-box qualification over the received bytes ———
    ...
    // ——— the extraction the boot-dependent legs share ———
    ...
    if (l1.exitCode === 0) {
      ...
      appPath = join(extractDir, `${PRODUCT_NAME}.app`);
    ...
    // ——— leg: registry-lease (two boots, hold/release/re-acquire) ———
    if (appPath !== null) {
    ...
    // ——— leg: packaged-smoke (H6's early-package family over the exact bytes) ———
    if (appPath !== null) {
    ...
    // ——— leg: native-better-sqlite3 (from source, executed under the artifact's node) ———
    if (appPath !== null) {
      ...
      (
        manifest.fixtures as { betterSqlite3: CandidateManifest['fixtures']['betterSqlite3'] }
      ).betterSqlite3 = sqlite.facts;
      await recordLeg(manifest, failures, {
  ...
  (manifest.minimumMacOS as ReturnType<typeof macOsClaim>) = macOsClaim(
    host,
    PRODUCT_MINIMUM_MACOS,
  );

  // ——— seal: verdict + the fail-closed completeness re-read ———
  (manifest as { finishedAt: string | null }).finishedAt = new Date().toISOString();
  (manifest as { verdict: CandidateManifest['verdict'] }).verdict = {
...
/** A log sink for one leg: a write function teeing into the leg's log file (sequentially, never interleaved). */
function logSink(manifestDir: string, leg: string): (chunk: string) => void {
  ...
  return (chunk: string): void => {
    ...
    pending = pending.then(() => writeFile(path, text)).catch(() => undefined);
  };
}
...
async function recordLeg(
  ...
  (manifest as { matrix: readonly MatrixLegRecord[] }).matrix = next;
