 ...
 *                                   the JSON reporter, then validates the
  ...
  const walk = (suite, titlePath, depth) => {
    ...
    for (const spec of suite.specs ?? []) {
      ...
      for (const test of spec.tests) {
        ...
        cases.push({ id: caseIdOf(entry), ...entry });
      }
    }
...
export function validateCheckpoint(input) {
  ...
  if (report === null && typeof input.reportText === 'string') {
    ...
    if (report === null) {
      push('malformed-report', 'the JSON report is `null`');
    }
  }
  if (report === null && findings.every((finding) => finding.code !== 'malformed-report')) {
    push('malformed-report', 'the JSON report is absent (a killed run produces no report)');
  }
  ...
  if (report !== null) {
    try {
      ({ cases, stats } = deriveCases(report, input.repoRoot ?? ROOT));
    } catch (error) {
  ...
  if (inventory !== null && input.updateInventory !== true) {
    ...
    const unregistered = reportedSorted.filter((id) => !registered.has(id));
...
/** Runs the full battery with the JSON reporter writing to `reportPath`. */
