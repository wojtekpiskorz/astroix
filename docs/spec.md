...
The full contracts live in the ADRs; each decision below names its authority. Anything not listed here retains its integration-era contract where the verticals are concerned (auto-write doctrine, route resolution, raw truth/raw field/zod projection) — those editing-domain contracts were extracted as behavior contracts during migration and bind the replacement unchanged; new-rule and override placement (nearest home, overrides fallback) is not among them, deferred beyond the pre-alpha per [#203](https://github.com/wojtekpiskorz/astroix/issues/203).
