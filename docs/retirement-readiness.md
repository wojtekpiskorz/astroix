...
| `target:staging-scripts` | `prepare-local-link.mjs`, `prepare-pack-fixture.mjs`, `prepare-src-link.mjs` | ADR-0001's three delivery lanes die at the gate (ADR-0010); their only remaining consumer dies with them | yes |
...
| `target:oracle-machinery` | `e2e/oracle.mjs`, `e2e/oracle.d.mts`, `e2e/contract-oracle/**` | its only consumers are the freeze specs + readiness leg 6, both dying with the runtime that boots the oracle; the corpora remain the frozen standard | **no — G3** |
