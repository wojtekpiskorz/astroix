...
- **Fail-closed on detach**: a failure to attach, enable, set, or retain the bypass makes the editing target **unready and disables edits**. Opening DevTools detaches the debugger, so **DevTools cannot share the authoritative editing target in version 1**. Recovery requires a fresh or reloaded target with the bypass restored before project content can become authoritative again.
