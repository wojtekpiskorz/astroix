...
- Mutations require exact `Host` and `Origin`, the correct host capability, the correct launcher/editor client role where applicable, JSON content, and `X-Astroix-Request: 1`. Reads require exact `Host`, the capability, same-origin Fetch Metadata, and a client role permitted for that resource. No CORS grant. SSE — a same-origin GET stream — follows the reads law: exact `Host`, the capability, same-origin Fetch Metadata, a client binding, and the current `SessionRef`; `Origin` is verified only when present, exactly as for any read. The Vite HMR upgrade validates exact `Host`/`Origin`, the active route, the activation capability, and Vite's token/subprotocol contract before proxying. *Amended 2026-09-03 (owner ruling on [#330](https://github.com/wojtekpiskorz/astroix/issues/330)): the SSE sentence originally demanded the exact `Origin`; that strict law 403'd every real client — `Origin` is a forbidden header on a same-origin GET in real browsers (fetch and `EventSource` alike), so its absence is the honest same-origin shape, and the raw-socket test pins that masked the mismatch were merely the present-and-exact case. SSE admission now aligns with the reads law.*
