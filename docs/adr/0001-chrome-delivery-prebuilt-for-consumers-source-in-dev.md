...
- `dependencies` is defined by reachability, not by folders (#138): exactly the packages node-side code must resolve at runtime — asserted by the `check:dist-graph` CI gate over `dist/` bare imports (builtins + `dependencies` + peers only) — while chrome-only libraries are `devDependencies` bundled into `dist/chrome.js`, and node-side build-time utilities (e.g. `yaml`) bundle into `dist/index.js` instead of shipping as consumer install weight.
