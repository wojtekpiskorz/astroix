...
The loop is automated by the **`Pre-alpha candidate` workflow** (`.github/workflows/pre-alpha-candidate.yml`, #259): dispatch-only, it assembles the exact pinned candidate, uploads it as a restricted draft release, downloads it back as a tester would receive it, and runs the complete qualification matrix over those exact bytes (`npm run candidate`), emitting the immutable evidence manifest as the run's artifact. A failed run forces a NEW candidate label — the evidence bundle is written once per label, and a failed upload path needs a new label anyway (`gh release create` fails on the existing tag, fail-closed by construction).
